  ...
  for (i = 0; i < R->nnodes; i++)
    {
      if (i == dest)
        continue;
      for (vn = vector_head (R->route[i][dest].nexthops); vn;
           vn = vector_next (vn))
        {
          ...
          if (nexthop == NULL)
            continue;
          os = node_get (i, R->G);
          ot = node_get (nexthop->node->id, R->G);
          s = node_copy_create (os, G);
          t = node_copy_create (ot, G);
          link = link_get (s, t, G);
          link->probability = nexthop->ratio;
        }
    }
}
...
DEFINE_COMMAND (sort_input_output_links,
                "sort input-output-links",
                "sort\n"
                "ilinks & olinks vector in the node\n")
{
  struct shell *shell = (struct shell *) context;
  struct graph *g = (struct graph *) shell->context;
  /* XXX */
...
graph_init ()
{
  ...
  INSTALL_COMMAND (cmdset_graph, import_ospf_graph);
