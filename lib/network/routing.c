  ...
  for (i = 0; i < routing->nnodes; i++)
    {
      if (i == destnode->id)
        continue;
      for (vn = vector_head (routing->route[i][destnode->id].nexthops); vn;
           vn = vector_next (vn))
        {
          struct nexthop *nexthop = (struct nexthop *) vn->data;
          if (nexthop == NULL)
            continue;
          os = node_get (i, routing->G);
          ot = node_get (nexthop->node->id, routing->G);
          s = node_copy_create (os, graph);
          t = node_copy_create (ot, graph);
          link = link_get (s, t, graph);
